...
import { PDFReportGenerator, type ReportData } from '@/lib/pdf/report-generator';
import type { Session, Agent, Round, Responce as AgentResponse, Report } from '@/database.types';
  ...
  try {
    ...
    const sessionData = {
      ...
      rounds: (rounds || []).map(round => ({
        ...
        responses: (round.agent_responses || []).map(response => ({
          agentName: response.agents?.name || 'Unknown Agent',
          response: response.response_text || '',
          reasoning: response.reasoning,
        })),
        validation: null, // TODO: Add validation data if available
        userFeedback: null, // TODO: Add user feedback if available
      })),
    };

    // Generate report using AI
    const reportGenerator = new ReportGeneratorAgent();
    const reportResult = await reportGenerator.generateReport(sessionData, reportType);
