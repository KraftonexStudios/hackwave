...
import type { AgentResponse, ValidationResult, AgentConfig } from '@/lib/ai/gemini-client';

// Choose AI provider based on environment variable
const AI_PROVIDER = process.env.AI_PROVIDER || 'gemini'; // 'gemini' or 'groq'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { query, selectedAgents, sessionId }: ProcessRequest = await request.json();
    ...
    const validationResults: ValidationResult[] = await validateResponses(agentResponses, query);
...
async function generateAgentResponse(agent: any, query: string, context?: string): Promise<AgentResponse> {
  ...
  try {
    return await aiClient.generateAgentResponse(agentConfig, query, context);
  } catch (error) {
...
function analyzeSentiment(text: string): 'positive' | 'negative' | 'neutral' {
  ...
  const positiveWords = ['support', 'benefit', 'advantage', 'positive', 'effective', 'success', 'improvement', 'valuable'];
  const negativeWords = ['oppose', 'risk', 'problem', 'negative', 'ineffective', 'failure', 'concern', 'disadvantage'];
...
async function validateResponses(responses: AgentResponse[], query: string): Promise<ValidationResult[]> {
  try {
    return await aiClient.validateResponses(responses, query);
  } catch (error) {
    ...
    const validationTemplates = [
      {
        ...
        evidence: "Arguments follow clear logical structure with premises leading to conclusions",
        logicalFallacies: [],
        supportingFacts: ["Structured reasoning", "Clear premise-conclusion flow"]
      },
      {
        claim: "Quality and reliability of evidence cited",
        evidence: "Evidence sources vary in quality; some claims well-supported, others require additional verification",
        logicalFallacies: responses.some(r => r.confidence < 50) ? ["Low confidence indicators"] : [],
      ...
      },
      {
        ...
        logicalFallacies: [],
        supportingFacts: responses.map(r => `${r.agentName}: ${r.sentiment} perspective (${r.confidence}% confidence)`)
      },
      {
        claim: "Reasoning quality and logical structure",
        evidence: "Reasoning chains provided with varying levels of detail and logical rigor",
        logicalFallacies: responses.filter(r => r.reasoning.length === 0).length > 0 ? ["Insufficient reasoning provided"] : [],
        supportingFacts: responses.filter(r => r.reasoning.length > 0).map(r => `${r.agentName} provided reasoning chain`)
      }
    ];
