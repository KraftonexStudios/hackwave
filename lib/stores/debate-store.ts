...
export interface DebateFlowNode {
  id: string;
...
export interface DebateFlowEdge {
  id: string;
  source: string;
  target: string;
...
export interface DebateState {
  ...
  // Flow visualization
  flowNodes: DebateFlowNode[];
  flowEdges: DebateFlowEdge[];
...
export interface DebateActions {
  ...
  // Flow visualization
  setFlowNodes: (nodes: DebateFlowNode[]) => void;
  setFlowEdges: (edges: DebateFlowEdge[]) => void;
  updateFlowNode: (id: string, updates: Partial<DebateFlowNode>) => void;
  addFlowNode: (node: DebateFlowNode) => void;
  addFlowEdge: (edge: DebateFlowEdge) => void;
...
export const useDebateStore = create<DebateStore>()
  (devtools(
    subscribeWithSelector((set, get) => ({
      ...
      addSessionAgent: (sessionAgent) => set((state) => ({ 
        sessionAgents: [...state.sessionAgents, sessionAgent] 
      })),
      removeSessionAgent: (id) => set((state) => ({
      ...
      addProcessingAgent: (agentId) => set((state) => ({
        processingAgents: state.processingAgents.includes(agentId) 
          ? state.processingAgents 
          : [...state.processingAgents, agentId]
      })),
      removeProcessingAgent: (agentId) => set((state) => ({
        processingAgents: state.processingAgents.filter(id => id !== agentId)
      })),
...
export const useCurrentSessionRounds = () => 
  useDebateStore((state) => 
    state.rounds.filter(round => round.session_id === state.currentSession?.id)
  );

export const useCurrentSessionResponses = () => 
  useDebateStore((state) => {
    const currentRounds = state.rounds.filter(round => 
      round.session_id === state.currentSession?.id
    );
